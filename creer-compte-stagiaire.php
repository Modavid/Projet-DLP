...
include('db_config.php');
	...
	if(isset($_GET['sid']) )
	{
		...
		$SQL="SELECT * FROM stagiaire WHERE sid='$sid'";
		...
		if(!$result || mysql_num_rows($result)==0)
				{	?>
					...
					<h1>Vous n'étes pas un stagiaire.</h2> 
					<p>S'il y a une erreur, veuillez contacter l'administrateur afin qu'il vous identifie.</p><br>
					<p>Merci.</p>
				...
				else{ //c'est un stagiaire
				
				
                                echo "Voici l'id: ".$sid;
				$SQL3="SELECT * FROM utilisateurs WHERE sid='$sid'";
				$result3=mysql_query($SQL3);
					if(!$result3||mysql_num_rows($result3)==0) // On verifie si cette personne n'a pas déja créer un compte.
					{
					$SQLph="SELECT * FROM stagiaire WHERE sid='$sid'";
