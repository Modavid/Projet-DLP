	...
	else if($_SESSION['AUTH']==2 || $_SESSION['AUTH']==1)
	{
		...
		else
		{
			*/$row1=mysql_fetch_array($result1);
			...
			{
				if(isset($_POST['titre']) && isset($_POST['texte']) && isset($_POST['cuid']))
				{
					$texte =htmlspecialchars($_POST['texte']);
					$titre =htmlspecialchars($_POST['titre']);
					$cuid =htmlspecialchars($_POST['cuid']);
					...
					{		
							if(isset($_FILES["myfile"]))
							{
								...
								if($error<=0)
								{
									$SQL="UPDATE cultiver SET photo='$photo' WHERE cuid='$cuid'";
									$result = mysql_query($SQL);
								}
									move_uploaded_file($temp,"uploaded/".$name);
									$SQL="UPDATE cultiver SET titre='$titre', texte='$texte' WHERE cuid='$cuid'";
									$result = mysql_query($SQL);
