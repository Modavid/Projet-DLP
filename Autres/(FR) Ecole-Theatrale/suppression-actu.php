...
<body>
	...
	<?php
		...
		if(isset($_GET['aid']) && isset($_SESSION['AUTH']) == 1)//ADMIN
		{
			...
			if(mysql_num_rows($result)==0)
			{
				?>
					<script>
						alert('Cette actualité n\'existe pas !');
					</script>
				<?php
			}
			else
			{
				$SQL="DELETE FROM actualite WHERE aid='$aid'";
				$result = mysql_query($SQL);
				...
				if($result)
				{
					?> 
						<p>
							<h2 style="padding-left: 30px">Actualité supprimé<h2>
						</p>
					<?php
				}
				else
