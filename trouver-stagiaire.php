...
include('db_config.php');
	...
	if(isset($_POST['nom']) || isset($_POST['prenom']))
	{
		...
		$SQL="SELECT * FROM stagiaire WHERE nom='$nom' && prenom='$prenom' ";
		$result=mysql_query($SQL);
		if(!$result||mysql_num_rows($result)==0)
		{
			$SQL="SELECT * FROM stagiaire WHERE nom='$nom' || prenom='$prenom'  && nom!='' && prenom !=''";
			$result=mysql_query($SQL);
		}
		if(!$result||mysql_num_rows($result)==0)
		{
            ...
            <h3>Pour retourner à page précédente, <a href="form-creer-compte-stagiaire.php" ><U> cliquer ici</U></a></h3>
               </div> 
		          </div>
		     	</section>
			<?php
		...
		else if(mysql_num_rows($result)==1) // un seul eleve trouvé
		{
            ...
            Si vous n'êtes pas sur ces photos, c'est que votre profil n'a pas été crée par l'administrateur, merci de nous contacter par mail...</b></p>
				...
				<td type="eleve">
					...
					<a href="creer-compte-stagiaire.php?sid=<?php echo $sid;?>">
						<img type="eleve" src="uploaded/<?php echo $photo;?>" alt="photo-demain-le-primtemps">
					</a>
				<br>
				
				<!-- nom et prenom -->
					<a href="creer-compte-stagiaire.php?sid=<?php echo $sid;?>&photo=<?php echo $photo; ?>"><?php echo ucwords(strtolower(html_entity_decode($nom)))?><br><?php echo html_entity_decode($prenom);?></a>
				</td>
		...
		else // eleves multiples
		{
			...
			while($row = mysql_fetch_array($result))
			{
				...
				?>
					...
					<!-- nom et prenom -->
                                        <a href="creer-compte-stagiaire.php?sid=<?php echo $sid;?>"><?php echo ucwords(strtolower(html_entity_decode($nom)))?><br><?php echo html_entity_decode($prenom);?></a>
					</td>
