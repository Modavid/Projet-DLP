...
include("db_config.php");
	...
	else if($_SESSION['AUTH']==2 || $_SESSION['AUTH']==1)
	{
			...
			{
				if(isset($_POST['nom']) && isset($_POST['url'])  && isset($_POST['type']))
				{			
							$lnom  = $_POST['nom'];
							$nom  = mysql_real_escape_string($lnom);
							$url  = $_POST['url'];
							$type = $_POST['type'];
							$role = "partenaire";

							$SQL="SELECT nom, url, type FROM partenaire WHERE nom='$nom' && url='$url' && type='$type'";
							...
							if($row < 1 && isset($_FILES["myfile"]))
							{
								...
								{
									move_uploaded_file($temp,"uploaded/partenaire/".$name);
									$SQL="INSERT INTO partenaire(nom,url,photo,type) VALUES ('$nom','$url','$name','$type')";
									$result = mysql_query($SQL);
									
									$SQL2="SELECT * FROM partenaire WHERE nom='$nom' and url='$url' and type='$type'";
